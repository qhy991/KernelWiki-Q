...
Write a CUDA kernel for Q4_0 quantized GEMM that achieves much better performance than the baseline (currently only 0.6% of baseline). The kernel computes C = A @ W^T where A(M,K) is FP32 activation, W(N,K/32) is Q4_0...
