...
def write_manifest(pages):
    """Write the common machine-readable manifest and human index."""
    records = []
    for p in sorted(pages, key=lambda x: x.get("id", x["_path"])):
        records.append({
            "id": p.get("id"),
            "type": p.get("type"),
            "title": p.get("title"),
...
def main():
    ...
    print("  Generated queries/pages.json")
    print("  Generated queries/INDEX.md")
    print("Done.")
